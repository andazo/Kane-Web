...
export const Inactives = () => {
  const numberOfRows = 10;
  const [inactiveCollaborators, setInactiveCollaborators] = useState();
  ...
  function checkInactive(driver) {
    return driver.status.id == 0;
  }
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        setInactiveCollaborators(response.data.filter(checkInactive));
        setTimeout(() => {
          setLoading(!loading);
        }, 100);
      })
  ...
  return (
    <>
      ...
      {!loading && (
        <ReportsList
          title="Reporte de Taxistas Inactivos"
          numberOfRows={numberOfRows}
          columns={columns}
          data={inactiveCollaborators}
        />
      )}
